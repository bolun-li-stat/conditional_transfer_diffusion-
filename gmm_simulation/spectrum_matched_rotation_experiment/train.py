...
from utils import device_from_name, setting_id, upsert_csv
...
def train_one(cfg: ExperimentConfig, model_type: str, split: dict[str, np.ndarray],
              diffusion: DDPM, initial_state: dict[str, torch.Tensor], resume: bool) -> tuple[ConditionalDenoiser, float, Path]:
    sid = setting_id(cfg.seed, cfg.capacity, cfg.rotation_deg, model_type, cfg.training_steps,
                     cfg.d, cfg.spectrum.lambda_high, cfg.spectrum.lambda_low)
    ...
    for _ in range(start, cfg.training_steps):
        ...
        if (_ + 1) % 1000 == 0:
            ...
            torch.save({"model": model.state_dict(), "optimizer": optimizer.state_dict(),
                        "step": _ + 1, "generator_state": gen.get_state()}, checkpoint)
    checkpoint.parent.mkdir(parents=True, exist_ok=True)
    torch.save({"model": model.state_dict(), "optimizer": optimizer.state_dict(),
                "step": cfg.training_steps, "generator_state": gen.get_state()}, checkpoint)
...
def run_setting(cfg: ExperimentConfig, model_type: str, skip_generation: bool = False,
    ...
    if metrics_path.exists() and not force and sid in pd.read_csv(metrics_path)["setting_id"].astype(str).values:
        return {}
    ...
    model, final_loss, checkpoint = train_one(cfg, model_type, split, diffusion, initial_state, resume)
    ...
    row = {"setting_id": sid, "seed": cfg.seed, "model_type": model_type, "capacity": cfg.capacity,
           ...
           "lambda_high": cfg.spectrum.lambda_high, "lambda_low": cfg.spectrum.lambda_low,
...
def main() -> None:
    ...
    completed_target: set[tuple[int,str]] = set()
    for cfg in configs:
