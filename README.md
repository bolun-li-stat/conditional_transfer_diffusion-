...
The synthetic data are `d = 100` dimensional Gaussian mixtures with `K = 8` components by default. Component 1 is the target component. The target mean is zero and auxiliary means are random unit directions scaled to Euclidean separation `Delta = 20` with a minimum pairwise distance check.
...
* **Scenario A / shared covariance**: all components share `rho = 0.2` or `rho = -0.15`.
* **Scenario B / covariance mismatch**: target has `rho = 0.2`, auxiliaries use mismatch levels `0`, `mild`, `medium`, or `strong`.
...
* `n_target_train in {100, 250, 500}`
* `n_aux_train = 1000` for each auxiliary class
* seeds `{0, 1, 2}`
...
All metrics are saved to CSV under `results/`.
...
python plot_results.py --results-dir results --make-pca
```
...
Figures are written to `results/figures/`. Aggregated metrics are written to `results/aggregated_metrics.csv`.
...
6. Save outputs frequently under Drive-backed `results/` if you are worried about disconnects.
...
```text
experiment_type,covariance_scenario,rho,mismatch_level,K,d,Delta,n,
n_target_train,n_aux_train,seed,model_type,sampling_mode,training_steps,
